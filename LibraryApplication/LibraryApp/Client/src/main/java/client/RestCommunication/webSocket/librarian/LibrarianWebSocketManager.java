...
public class LibrarianWebSocketManager {
    private static LibrarianWebSocketManager instance;
    private final CopyOnWriteArrayList<WebSocketMessageListener> listeners = new CopyOnWriteArrayList<>();
    private boolean connected = false;

    private LibrarianWebSocketManager() {
    }

    public static synchronized LibrarianWebSocketManager getInstance() {
        if (instance == null) {
            instance = new LibrarianWebSocketManager();
        }
        return instance;
    }
    ...
    public synchronized void connect() {
        if (connected) return;
        String url = "ws://localhost:55555/librarian-websocket";
        ...
        try {
            ...
            StompSession session = stompClient.connectAsync(url, new StompSessionHandlerAdapter() {
                ...
                public void afterConnected(StompSession session, StompHeaders connectedHeaders) {
                    ...
                    connected = true;
                    subscribeToTopic(session, "/topic/librarian");
                }
